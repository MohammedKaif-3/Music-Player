...
public class DownloadsRecyclerViewAdapter extends RecyclerView.Adapter<DownloadsRecyclerViewAdapter.MyViewHolder> implements MusicPlayerService.OnSongChangeListener{
    ...
    public DownloadsRecyclerViewAdapter(Context context, ArrayList<Song> songsList) {
    ...
    private final ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            musicPlayerService.addOnSongChangeListener(DownloadsRecyclerViewAdapter.this);
    ...
    public void onDetachedFromRecyclerView(@NonNull RecyclerView recyclerView) {
        if (isBound) {
            musicPlayerService.removeOnSongChangeListener(DownloadsRecyclerViewAdapter.this);
    ...
    @Override
    public void onBindViewHolder(@NonNull DownloadsRecyclerViewAdapter.MyViewHolder holder, int position, @NonNull List<Object> payloads) {
    ...
    private void updateViewHolder(DownloadsRecyclerViewAdapter.MyViewHolder holder, MediaMetaDataHelper.MediaMetadata metadata) {
    ...
    private void updateCurrentSongUI(DownloadsRecyclerViewAdapter.MyViewHolder holder, String songPath) {
    ...
    public int findPositionByPath(String targetPath) {
        for (int i = 0; i < songsList.size(); i++) {
            if (songsList.get(i).getPath().equals(targetPath)) {
    ...
    @NonNull
    @Override
    public DownloadsRecyclerViewAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.row_item, parent, false);
        pathsList = new ArrayList<>();
        for (Song song1 : songsList) {
            pathsList.add(song1.getPath()); // Assuming Song class has getPath() method
        }
        return new DownloadsRecyclerViewAdapter.MyViewHolder(view);
    }
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        // Use a thread pool to extract metadata
        ...
        holder.itemView.setOnClickListener(v -> {
            if(song.getPath().equals(currentSongPath)){
                ...
                Executors.newSingleThreadExecutor().execute(() -> {
                    ...
                    MusicPlayerInstance.setPlayingQueue(2);
